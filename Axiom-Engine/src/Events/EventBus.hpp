...
namespace Axiom {
	...
	class AXIOM_API EventBus {
	public:
		using Callback = std::function<void(AxiomEvent&)>;
		...
		EventId Subscribe(F&& callback) {
			return Subscribe([fn = std::forward<F>(callback)](AxiomEvent& event) mutable {
