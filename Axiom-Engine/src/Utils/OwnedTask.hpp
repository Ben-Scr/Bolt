...
#include <stop_token>
#define AXIOM_HAS_STD_JTHREAD 1
#else
#define AXIOM_HAS_STD_JTHREAD 0
#endif
	...
	private:
		...
		static void LogUnhandledException(const std::exception_ptr& exception) noexcept
		{
			...
			catch (const std::exception& e)
			{
				try
				{
					AIM_CORE_ERROR_TAG("OwnedTask", "Background task failed: {}", e.what());
				}
				catch (...)
				{
				}
			...
			catch (...)
			{
				try
				{
					AIM_CORE_ERROR_TAG("OwnedTask", "Background task failed with an unknown exception");
				}
				catch (...)
				{
				}
