...
#define AIM_NATIVE_LOG_INFO(msg)  do { if (Axiom::g_EngineAPI && Axiom::g_EngineAPI->LogInfo) Axiom::g_EngineAPI->LogInfo(msg); } while(0)
#define AIM_NATIVE_LOG_WARN(msg)  do { if (Axiom::g_EngineAPI && Axiom::g_EngineAPI->LogWarn) Axiom::g_EngineAPI->LogWarn(msg); } while(0)
#define AIM_NATIVE_LOG_ERROR(msg) do { if (Axiom::g_EngineAPI && Axiom::g_EngineAPI->LogError) Axiom::g_EngineAPI->LogError(msg); } while(0)

#define REGISTER_SCRIPT(ClassName) \
	static struct ClassName##_AutoReg { \
		ClassName##_AutoReg() { Axiom::NativeScriptRegistry::Register(#ClassName, []() -> Axiom::NativeScript* { return new ClassName(); }); } \
